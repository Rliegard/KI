###########################################################################################
# PROJEKT: Wissens-KI KI.M7 (FINALER FIX: Umstellung auf Deep-Translator für stabile Übersetzung)
# DATEI:   wissens_ki_m7.py
#
# BESCHREIBUNG:
# Diese Version ersetzt die instabile googletrans-Bibliothek durch deep_translator,
# um den persistenten AttributeError und das RuntimeWarning zu beheben und die
# Übersetzungsstabilität zu gewährleisten. Die Pausen wurden beibehalten, um
# IP-Blockaden zu vermeiden.
...
MIN_TEXT_LENGTH = 150
# Blockgröße leicht reduziert, um Puffer für Satzzeichen zu haben, aber hoch genug für Effizienz
...
def translate_to_german(text):
    ...
    Zerlegt große Texte in Blöcke und fügt Pausen ein, um Rate-Limit-Fehler zu vermeiden.
    """
    if not text:
        return ""

    # Initialisiere den Übersetzer einmalig
    translator = GoogleTranslator(source='auto', target='de')
    text_blocks = []
    current_block = ""

    # Text in Sätze (oder annähernd Sätze) zerlegen
    ...
    print(f"INFO: Starte robuste Übersetzung von {len(text_blocks)} Textblöcken (mit deep-translator)...")

    for i, block in enumerate(text_blocks):
        try:
            # NEUE SYNTAX für deep_translator
            translation = translator.translate(block)
            translated_text.append(translation)

            if len(text_blocks) > 1:
                # Pause leicht verlängert für mehr Stabilität (0.5 bis 1.5 Sekunden)
                time.sleep(random.uniform(0.5, 1.5))
        except Exception as e:
            # Fehlerfall: Den Original-Text des Blocks als Fallback hinzufügen
            print(f"[Übersetzungsfehler Block {i+1} - {type(e).__name__}: Verwende Originaltext.]")
            translated_text.append(block)
            time.sleep(random.uniform(0.5, 1.0)) # Kurze Pause, um den Dienst nicht zu überlasten
...
def summarize_multiple_sources(sources_data, anfrage):
    ...
    for i, data in enumerate(sources_data):
        ...
        relevant_sentences = []
        for sentence in sentences:
            relevance_score = len(sentence) / 100
...
def get_text_from_url(url, current_proxy=None):
    ...
    Verbesserte Header-Rotation und Proxy-Nutzung zur Anti-Detection.
    """
    ...
    try:
        ...
        # Gib True zurück, wenn der Fehler nicht 403/404 ist, damit der Whitelist-Vergleich trotzdem versucht wird
...
def ki_wissensabruf_und_vergleich(anfrage, quelle_typ, stop_search_flag):
    """
    Führt eine Suche durch mit 1x DDGS und den anschließenden
    Quellenvergleich im Whitelist-Fallback.
    Übernimmt das stop_search_flag zum Abbruch.
    """
    ...
    for retry_count in range(MAX_RETRIES):
        ...
        try:
            # DDGS wird mit dem ausgewählten Proxy und dem timeout=20 aufgerufen
            ...
            for i, result in enumerate(results):
                ...
                # get_text_from_url wird mit demselben Proxy aufgerufen
    ...
    if not successful_content:
        ...
        for base_url in RELIABLE_URL_WHITELIST:
            ...
            # Wählt einen zufälligen Proxy für jede Whitelist-Anfrage
            ...
            elif domain_name in ["www.nasa.gov", "www.nih.gov", "www.epa.gov", "www.eia.gov", "www.usgs.gov"]:
                final_url = f"{base_url}search?q={suchstring_query}"
            elif domain_name in ["www.nature.com", "www.sciencemag.org", "www.science.org"]:
                final_url = f"{base_url}search?q={suchstring_query}"
            ...
            # get_text_from_url mit rotiertem Proxy aufrufen
        ...
        # Verarbeite gesammelte Whitelist-Ergebnisse
        if whitelist_results:
            dienst_name = "Whitelist-Quellenvergleich"

            for item in whitelist_results:
                # Prüfen, ob während der Übersetzung abgebrochen wurde
                if stop_search_flag.is_set():
                    return "Suche durch den Benutzer abgebrochen.", "Abbruch"
                # Übersetzung mit deep-translator
    ...
    # --- 3. ERGEBNIS GENERIEREN UND BEGRENZEN (MAX_CHARS) ---

    if successful_content:
        # KORRIGIERT: NameError behoben und auf "Abbruch" prüfen
        ...
        # **FALL A: DDGS war erfolgreich**
        if dienst_name != "Whitelist-Quellenvergleich":

            # Die Übersetzung findet hier statt, bevor der Text in die GUI kommt
            ...
            # Begrenzung des Textes auf MAX_CHARS (5000)
...
class WissensKI_GUI:
    ...
    def __init__(self, master):
        ...
        # NEU: Flag für den Abbruch des Such-Threads
        ...
        # Start im Vollbildmodus (kann mit Escape oder Strg+Q beendet werden)
        ...
        # SPEICHER-BUTTON
        self.speichern_button = ttk.Button(button_frame, text="Ergebnis Speichern", command=self.speichere_ergebnis, state='disabled', style='Accent.TButton')
        self.speichern_button.grid(row=0, column=2, padx=5, sticky=(tk.W, tk.E))
        Tooltip(self.speichern_button, "Speichert das aktuell angezeigte Ergebnis in der Datenbank.")
    ...
    def starte_suche_thread(self):
        ...
        self.stop_search_flag.clear() # Flag zurücksetzen
        ...
        # Starte den Such-Thread und übergebe das Abbruch-Flag
    ...
    def aktualisiere_ausgabe(self, ergebnis, anfrage):
        ...
        if not ergebnis.startswith("Keine Online-Dokumente") and not ergebnis.startswith("Suche wurde erfolgreich"):
...
if __name__ == "__main__":
    ...
    style.configure('Accent.TButton', foreground='white', background='#0078D4', borderwidth=1)
