#################################################################################################
# PROJEKT: Wissens-KI M4 - Erweiterte Version
...
# 2. Quellen-Filterung: Suchanfragen können auf spezifische Quellentypen
#    ("Wissen", "Forschung", "Allgemein") zugeschnitten werden.
...
# 🟢 ERWEITERT UND KORRIGIERT: Statische Liste zuverlässiger, spezialisierter URLs (WHITELIST)
RELIABLE_URL_WHITELIST = [
    # Allgemeine und Bildung
    "https://de.wikipedia.org/wiki/",           # Deutsche Wikipedia (Direkte Artikelsuche)
    ...
    # Python/Coding
    "https://docs.python.org/3/",               # Offizielle Python-Dokumentation (Englisch, sehr aktuell)
...
# 🟢 ANGEPASST: Quelle_typ wird ignoriert, da die Dropdown entfernt wurde.
def ki_wissensabruf_und_vergleich(anfrage, quelle_typ):
    ...
    # quelle_typ wird hier zu "Allgemeine Suche" gesetzt, da das Dropdown entfernt wurde
    ...
    # --- 1. DDGS SUCH-STRATEGIEN (Jetzt nur allgemeine Strategie) ---
    ...
    if not successful_content:
        ...
        suchstring_wiki = anfrage.replace(" ", "_")
        ...
        for base_url in RELIABLE_URL_WHITELIST:
            ...
            # Spezialbehandlung für offizielle Python-Dokumentation (Suche innerhalb der Docs)
            ...
            # Warten, da wir direkt auf externe Seiten zugreifen
    ...
    if successful_content:
        ...
        # Titelanzeige der Whitelist-Quellen verbessern
...
class WissensKI_GUI:
    ...
    def __init__(self, master):
        ...
        # 🟢 NEU: Setze das Fenster in den Vollbildmodus
        ...
        main_frame = ttk.Frame(master, padding="20") # Padding für Fullscreen erhöht
        main_frame.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        main_frame.columnconfigure(0, weight=1)
        main_frame.rowconfigure(4, weight=1) # Row-Index angepasst
        ...
        # 2. Suchen-Button
        self.suchen_button = ttk.Button(main_frame, text="Suchen (Ctrl+S)", command=self.starte_suche_thread, style='TButton')
        self.suchen_button.grid(row=2, column=0, columnspan=2, pady=10, sticky=(tk.W, tk.E))
    ...
    def starte_suche_thread(self):
        ...
        # Der quelle_typ ist nun hartkodiert als "Allgemeine Suche"
