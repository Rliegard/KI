###########################################################################################
# PROJEKT: Wissens-KI KI.M6
# DATEI:   wissens_ki_m6.py
#
# BESCHREIBUNG:
...
#    gespeichert (wissens_ki_cache.db).
...
def ki_wissensabruf_und_vergleich(anfrage, quelle_typ):
    ...
    if successful_content:
        ...
        # Fallback, falls die Logik zu kurz ist
...
class WissensKI_GUI:
    ...
    def __init__(self, master):
        ...
        # 🟢 Frame für den Beenden-Button, oben rechts
        top_right_frame = ttk.Frame(main_frame)
        top_right_frame.grid(row=0, column=1, sticky=tk.NE, padx=5, pady=5) # Oben rechts in main_frame

        # 🟢 Beenden-Button
        self.exit_button = ttk.Button(top_right_frame, text="Beenden (Ctrl+Q)", command=master.quit, style='TButton')
        self.exit_button.pack(side=tk.RIGHT)
        Tooltip(self.exit_button, "Beendet die Anwendung.")


        # 1. Eingabebereich (startet nun in Reihe 1, da Reihe 0 für den Beenden-Button genutzt wird)
        ttk.Label(main_frame, text="Ihre Anfrage:", font=('Arial', 14, 'bold')).grid(row=1, column=0, columnspan=1, sticky=tk.W) # Spalte 1 ist jetzt für den Button
        ...
        text_frame.grid(row=5, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S)) # Reihe 5
